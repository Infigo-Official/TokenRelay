...
.PARAMETER UsePublishProfile
    Use predefined publish profiles for optimized builds (StandaloneOptimized, LinuxOptimized, FrameworkDependent)
...
if ($UsePublishProfile) {
    Write-Host "🎯 Using optimized publish profiles..." -ForegroundColor Cyan
    
    # Determine which publish profile to use
    $PublishProfile = $null
    if ($SelfContained -and $Runtime -eq "win-x64") {
        $PublishProfile = "StandaloneOptimized"
    } elseif ($SelfContained -and $Runtime -eq "linux-x64") {
        $PublishProfile = "LinuxOptimized"
    } elseif (-not $SelfContained) {
        $PublishProfile = "FrameworkDependent"
    }
    
    if ($PublishProfile) {
        Write-Host "📋 Using publish profile: $PublishProfile" -ForegroundColor Cyan
        $PublishArgs = @(
            "publish"
            $ProjectDir
        ...
        Write-Host "⚠️  No matching publish profile found, using standard publishing..." -ForegroundColor Yellow
        $UsePublishProfile = $false
    }
}

if (-not $UsePublishProfile) {
    $PublishArgs = @(
        "publish"
        $ProjectDir
